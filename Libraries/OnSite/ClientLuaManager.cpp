/**
 *	ClientLuaManager Class Definition
 *	Terrell Ibanez
**/
...
//Constructor
OnSite::ClientLuaManager::ClientLuaManager() {
	//Create Lua State
	...
	//Load API
	luaL_loadfile(mlState, "ClientAPI.lua");
	
	int rCode = lua_pcall(mlState, 0, 0, 0);

	if (rCode != 0) {
		std::cout << lua_tostring(mlState, -1) << std::endl;
	}
}
...
#if defined(__ANDROID__)
//Load Lua Code into Lua on the Android platform.
int OnSite::ClientLuaManager::loadLuaDroid(lua_State *L, const char * filename)
{
	return loadLuaDroidR(L, filename);		
}

#else
//You should not be calling this function if you're not on android.
int OnSite::loadLuaDroid(lua_State *L, const char * filename)
{
	return -1;
}
#endif
...
void OnSite::ClientLuaManager::loadCode() {
	#if defined(__ANDROID__)
	loadLuaDroid(mlState, "ClientCode.lua");
	#else
	luaL_loadfile(mlState, "ClientCode.lua");
	#endif
...
void OnSite::ClientLuaManager::runGameLoop() {
	lua_getglobal(mlState, "OnSite");
	lua_getfield(mlState, -1, "Client");
...
void OnSite::ClientLuaManager::processIncomingMessagePacket(std::shared_ptr<OnSite::MessagePacket> pmPacket) {
...
void OnSite::ClientLuaManager::sendOutgoingMessagePackets(std::shared_ptr<OnSite::ConcurrentQueue<OnSite::MessagePacket>> pqOutgoing) {
	//Push Sync Table to Top of Stack
	lua_getglobal(mlState, "OnSite");
	lua_getfield(mlState, -1, "Client");
	lua_remove(mlState, -2);
...
std::tuple<std::tuple<double, double, double, double, double, double, double>*, size_t> OnSite::ClientLuaManager::getTextureMaps() {
